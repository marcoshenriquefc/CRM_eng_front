...
export async function mockApi<T>(url: string, opts: { method?: string; body?: unknown }): Promise<ApiResponse<T>> {
    ...
    if (url === '/auth/login' && method === 'POST')
        return ok({ user: mockUser, workspace: mockWorkspace, accessToken: 'mock_access_token' } as T)
    if (url === '/auth/refresh' && method === 'POST')
        return ok({ user: mockUser, workspace: mockWorkspace, accessToken: 'mock_access_token_refreshed' } as T)
