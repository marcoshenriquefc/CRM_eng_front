...
export async function apiRequest<T>(url: string, opts: Parameters<typeof $fetch>[1] & { accessToken?: string | null } = {}): Promise<ApiResponse<T>> {
  ...
  return $fetch<ApiResponse<T>>(url, {
    ...
    ...opts,
    headers: { ...(opts.headers || {}), ...(opts.accessToken ? { Authorization: `Bearer ${opts.accessToken}` } : {}) }
  })
}
